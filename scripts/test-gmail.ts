...
async function main() {
  ...
  try {
    // 購入通知メールを取得（1ヶ月以内・未読）
    // 実運用で使う送信元:
    // - minne: order@minne.com
    // - creema: info@creema.jp
    const query = 'is:unread newer_than:30d (from:order@minne.com OR from:info@creema.jp)';
    ...
    if (notifications.length === 0) {
      ...
      console.log(`\n検索条件: ${query}`);
      console.log(
        '\nヒント: order@minne.com / info@creema.jp の未読メールが過去30日以内にあるか確認してください',
      );
    } else {
