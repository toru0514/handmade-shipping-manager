...
「PR#NNをレビューして」と言われたら以下を実行する:

1. PRの差分・CI状況を確認してコードレビューを行う
2. レビュー内容をPRのコメントに投稿する
3. **不具合がなければマージする**（squash merge + delete branch）
4. **軽微でも不具合があればマージしない**
5. 「マージしないでください」と明示された場合はレビューコメントのみ投稿する

## PRレビュー修正対応

「レビューしました」「確認と修正して」と言われたら、該当PRのGitHubコメント（`gh api repos/{owner}/{repo}/pulls/{number}/reviews`）を確認し、指摘事項を修正する。
